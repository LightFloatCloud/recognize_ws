...
void LivoxMsgCallback(const livox_ros_driver2::CustomMsgConstPtr& livox_msg_in) {
...
PointsGroundFilter::PointsGroundFilter(ros::NodeHandle& nh)
{
    ...
    nh.param<int>("TO_MERGE_CNT", TO_MERGE_CNT, 1);


    
    sub_livox_msg = nh.subscribe<livox_ros_driver2::CustomMsg>(sub_topic_,5, LivoxMsgCallback);
    pub_pcl_out = nh.advertise<sensor_msgs::PointCloud2>("/livox_pcl", 10);


    sub_ = nh.subscribe("/livox_pcl", 1, &PointsGroundFilter::callback, this);
