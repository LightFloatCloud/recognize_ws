...
EuCluster::EuCluster(ros::NodeHandle &nh)
{
    ...
    ros::param::get("~crop_cluster_size", crop_cluster_size_);
...
void EuCluster::fitOrientedBoundingBox(const pcl::PointCloud<pcl::PointXYZI>::Ptr pc,
...
// 拟合无方向的box
void EuCluster::fitBoundingBox(const pcl::PointCloud<pcl::PointXYZI>::Ptr pc,
                               visualization_msgs::Marker& marker)
{
    // 拟合3D包围盒
...
void EuCluster::clusterInSegment(const pcl::PointCloud<pcl::PointXYZI>::Ptr pc,
                                 ...
                                 visualization_msgs::MarkerArray& objs)
{
    ...
    pcl::EuclideanClusterExtraction<pcl::PointXYZI> eucluster;
    ...
    {
        // 建立点云
        pcl::PointCloud<pcl::PointXYZI>::Ptr pc_sub(new pcl::PointCloud<pcl::PointXYZI>);
        ...
        std::vector<std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr>> pc_refined_array;
        pc_refined_array.resize(num_x, std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr>(num_y));
        ...
        for(size_t nx = 0; nx < num_x; nx++)
        {
            for(size_t ny = 0; ny < num_y; ny++)
            {
                ...
                /*----------------------修改部分--------------------*/
                if(marker.scale.x > crop_cluster_size_[0] || marker.scale.y > crop_cluster_size_[1] || marker.scale.z > crop_cluster_size_[2])
                {
                    continue;
                }
                
                size_t pointsize_submarker = pc_refined_array[nx][ny]->points.size();
                
                float avg_intensity = 0;
                for(size_t p = 0; p < pointsize_submarker; p++)
                {
                    avg_intensity += pc_refined_array[nx][ny]->points[p].intensity;
                }
                avg_intensity /= pointsize_submarker;
                
                
                marker.text = "AvgI:" + std::to_string(avg_intensity);
...
void EuCluster::cluster(const pcl::PointCloud<pcl::PointXYZI>::Ptr pc,
                        visualization_msgs::MarkerArray& objs)
{
    // 根据分区，定义子云
    std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> pc_array(seg_num_);
...
void EuCluster::crop(const pcl::PointCloud<pcl::PointXYZI>::Ptr pc,
                     const pcl::PointCloud<pcl::PointXYZI>::Ptr pc_cropped,
...
void EuCluster::callback(const sensor_msgs::PointCloud2ConstPtr in)
{
    ...
    pcl::PointCloud<pcl::PointXYZI>::Ptr pc_cropped(new pcl::PointCloud<pcl::PointXYZI>);
    ...
    if(road_info_)
    {
        ...
        /*----------------------------------*/
        // 从参数服务器中获取参数
        ros::param::get("~crop_distance", crop_distance_);
    ...
    for(size_t i = 0; i < objs.markers.size(); i++)
    {
        ...
        objs.markers[i].lifetime = ros::Duration(0.8);
        //objs.markers[i].text = ' ';
    ...
    /******************************************************************/
    int markersize = static_cast<int>(objs.markers.size() );
    for(int p = 0; p < std::min(markersize, 5); p++)
    {
        std::string str = objs.markers[p].text;
        float avg_I;
        if (str.substr(0, 5) == "AvgI:")
        {  // 检查字符串是否以前缀开头
            std::string suffix = str.substr(5);
            
            avg_I = std::stof(suffix);
            std::cout << "The float value is " << avg_I << std::endl;
